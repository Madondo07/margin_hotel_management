...
This repository contains the core backend application for **Margin Hotel**, an established hotel group with several branches across South Africa. The project transitions the hotel's traditional paper and spreadsheet-based front-desk operations into a modern, centralized digital system.
...
The scope of this project is **back-end only** (domain layer → REST controllers). The public booking website and the internal staff portal front-end views are out of scope and will be built by another development team.
...
- **Anti-Double-Booking Logic:** A room cannot be double-booked. No two bookings for the exact same room may overlap in their dates. This rule is strictly enforced within the service layer, not the UI.
